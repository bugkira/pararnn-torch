...
def _local_scan_kernel(
    ...
    j02 = _load_j(j_ptr, pid_b, offs_t, offs_d, 2, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j03 = _load_j(j_ptr, pid_b, offs_t, offs_d, 3, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j10 = _load_j(j_ptr, pid_b, offs_t, offs_d, 4, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j11 = _load_j(j_ptr, pid_b, offs_t, offs_d, 5, 1.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j12 = _load_j(j_ptr, pid_b, offs_t, offs_d, 6, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j13 = _load_j(j_ptr, pid_b, offs_t, offs_d, 7, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j20 = _load_j(j_ptr, pid_b, offs_t, offs_d, 8, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j21 = _load_j(j_ptr, pid_b, offs_t, offs_d, 9, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j22 = _load_j(j_ptr, pid_b, offs_t, offs_d, 10, 1.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j23 = _load_j(j_ptr, pid_b, offs_t, offs_d, 11, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j30 = _load_j(j_ptr, pid_b, offs_t, offs_d, 12, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j31 = _load_j(j_ptr, pid_b, offs_t, offs_d, 13, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j32 = _load_j(j_ptr, pid_b, offs_t, offs_d, 14, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j33 = _load_j(j_ptr, pid_b, offs_t, offs_d, 15, 1.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    ...
    _store_j(j_out_ptr, s02, pid_b, offs_t, offs_d, 2, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s03, pid_b, offs_t, offs_d, 3, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s10, pid_b, offs_t, offs_d, 4, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s11, pid_b, offs_t, offs_d, 5, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s12, pid_b, offs_t, offs_d, 6, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s13, pid_b, offs_t, offs_d, 7, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s20, pid_b, offs_t, offs_d, 8, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s21, pid_b, offs_t, offs_d, 9, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s22, pid_b, offs_t, offs_d, 10, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s23, pid_b, offs_t, offs_d, 11, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s30, pid_b, offs_t, offs_d, 12, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s31, pid_b, offs_t, offs_d, 13, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s32, pid_b, offs_t, offs_d, 14, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    _store_j(j_out_ptr, s33, pid_b, offs_t, offs_d, 15, mask, stride_ojb, stride_ojt, stride_ojk, stride_ojd)
    ...
    _store_r(r_out_ptr, u2, pid_b, offs_t, offs_d, 2, mask, stride_orb, stride_ort, stride_ors, stride_ord)
    _store_r(r_out_ptr, u3, pid_b, offs_t, offs_d, 3, mask, stride_orb, stride_ort, stride_ors, stride_ord)
    ...
    store_acc(agg_r_ptr + pid_b * stride_arb + pid_c * stride_arc + 0 * stride_ars + offs_d * stride_ard, tl.sum(tl.where(last, u0, 0.0), axis=0), dmask)
    store_acc(agg_r_ptr + pid_b * stride_arb + pid_c * stride_arc + 1 * stride_ars + offs_d * stride_ard, tl.sum(tl.where(last, u1, 0.0), axis=0), dmask)
    store_acc(agg_r_ptr + pid_b * stride_arb + pid_c * stride_arc + 2 * stride_ars + offs_d * stride_ard, tl.sum(tl.where(last, u2, 0.0), axis=0), dmask)
    store_acc(agg_r_ptr + pid_b * stride_arb + pid_c * stride_arc + 3 * stride_ars + offs_d * stride_ard, tl.sum(tl.where(last, u3, 0.0), axis=0), dmask)
...
def _chunk_incl_kernel(
    ...
    j00 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 0, 1.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j01 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 1, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j02 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 2, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j03 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 3, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j10 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 4, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j11 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 5, 1.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j12 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 6, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j13 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 7, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j20 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 8, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j21 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 9, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j22 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 10, 1.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j23 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 11, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j30 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 12, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j31 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 13, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j32 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 14, 0.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    j33 = _load_j(agg_j_ptr, pid_b, offs_c, offs_d, 15, 1.0, mask, stride_ajb, stride_ajc, stride_ajk, stride_ajd)
    ...
    u2 = _load_r(agg_r_ptr, pid_b, offs_c, offs_d, 2, mask, stride_arb, stride_arc, stride_ars, stride_ard)
    u3 = _load_r(agg_r_ptr, pid_b, offs_c, offs_d, 3, mask, stride_arb, stride_arc, stride_ars, stride_ard)
    (
    ...
    _store_r(incl_r_ptr, s2, pid_b, offs_c, offs_d, 2, mask, stride_ib, stride_ic, stride_is, stride_id)
    _store_r(incl_r_ptr, s3, pid_b, offs_c, offs_d, 3, mask, stride_ib, stride_ic, stride_is, stride_id)
...
def _apply_carry_kernel(
    ...
    j02 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 2, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j03 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 3, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j10 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 4, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j11 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 5, 1.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j12 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 6, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j13 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 7, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j20 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 8, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j21 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 9, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j22 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 10, 1.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j23 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 11, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j30 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 12, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j31 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 13, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j32 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 14, 0.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    j33 = _load_j(j_loc_ptr, pid_b, offs_t, offs_d, 15, 1.0, mask, stride_jb, stride_jt, stride_jk, stride_jd)
    ...
    r2 = _load_r(r_loc_ptr, pid_b, offs_t, offs_d, 2, mask, stride_rb, stride_rt, stride_rs, stride_rd)
    r3 = _load_r(r_loc_ptr, pid_b, offs_t, offs_d, 3, mask, stride_rb, stride_rt, stride_rs, stride_rd)
    c0 = load_acc(incl_r_ptr + pid_b * stride_ib + idx_c * stride_ic + 0 * stride_is + offs_d * stride_id, dmask, 0.0)
    c1 = load_acc(incl_r_ptr + pid_b * stride_ib + idx_c * stride_ic + 1 * stride_is + offs_d * stride_id, dmask, 0.0)
    c2 = load_acc(incl_r_ptr + pid_b * stride_ib + idx_c * stride_ic + 2 * stride_is + offs_d * stride_id, dmask, 0.0)
    c3 = load_acc(incl_r_ptr + pid_b * stride_ib + idx_c * stride_ic + 3 * stride_is + offs_d * stride_id, dmask, 0.0)
    ...
    _store_r(out_ptr, out2, pid_b, offs_t, offs_d, 2, mask, stride_ob, stride_ot, stride_os, stride_od)
    _store_r(out_ptr, out3, pid_b, offs_t, offs_d, 3, mask, stride_ob, stride_ot, stride_os, stride_od)
...
def scan_block4_triton(jac: Tensor, residual: Tensor) -> Tensor:
    ...
    validate_cuda_tensors(jac, residual, name="scan_block4_triton")
    jac = jac.contiguous()
