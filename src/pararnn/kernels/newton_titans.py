"""Fused ParaTitans Newton: cell + diagonal J + scan (Alg. 1).

``W_x(x)`` is ``(B, T, 5 d_h)`` = ``(α_pre, θ_pre, k, v, n_pre)``. Diagonal
gains are ``u`` (tanh residual) and ``r`` (recurrent mix into tanh). CUDA
fp16/fp32 (bf16 on CC ≥ 8.0); cell+scan algebra in fp32.
"""
...
@triton.jit
def _titans_pred_j(h_prev, alpha_pre, theta_pre, k, v, n_pre, u, r):
    """Titans L=1 memory step + diag J."""
    alpha = tl.sigmoid(alpha_pre)
    theta = _softplus(theta_pre)
    grad = (h_prev * k - v) * k
    h_lin = (1.0 - alpha) * h_prev - theta * grad
    n = _tanh(n_pre + r * h_prev)
    h_new = h_lin + u * n
    n_p = 1.0 - n * n
    j = (1.0 - alpha) - theta * k * k + u * n_p * r
    return h_new, j
...
def _load_wx(wx_ptr, pid_b, offs_t, offs_d, d_h, mask, sb, st, sd):
    ...
    alpha_pre = load_acc(base + offs_d[None, :] * sd, mask, 0.0)
    theta_pre = load_acc(base + (offs_d[None, :] + d_h) * sd, mask, 0.0)
    k = load_acc(base + (offs_d[None, :] + 2 * d_h) * sd, mask, 0.0)
    v = load_acc(base + (offs_d[None, :] + 3 * d_h) * sd, mask, 0.0)
    n_pre = load_acc(base + (offs_d[None, :] + 4 * d_h) * sd, mask, 0.0)
    return alpha_pre, theta_pre, k, v, n_pre


@triton.jit
def _titans_init_kernel(
    wx_ptr,
    h_ptr,
    u_ptr,
    r_ptr,
    ...
    h_new, _ = _titans_pred_j(h_prev, alpha_pre, theta_pre, k, v, n_pre, u, r)
    store_acc(
...
@triton.jit
def _titans_cell_local_scan_kernel(
    h_ptr,
    wx_ptr,
    u_ptr,
    r_ptr,
    ...
    u = load_acc(u_ptr + offs_d, dmask, 0.0)
    r = load_acc(r_ptr + offs_d, dmask, 0.0)
    h_new, j = _titans_pred_j(h_prev, alpha_pre, theta_pre, k, v, n_pre, u, r)
    residual = h_new - h
    if HAS_CU:
...
@triton.jit
def _titans_apply_update_kernel(
    h_ptr,
    j_loc_ptr,
    r_loc_ptr,
    incl_r_ptr,
...
def _newton_titans_fused_impl(
    ...
    *,
    ...
    """Alg. 1 for diagonal ParaTitans. Public entry: ``pararnn::newton_titans_fused``."""
    wx = wx.contiguous()
    u = u.contiguous()
    r = r.contiguous()
    batch, time, five_d = wx.shape
    d_h = u.numel()
    if r.numel() != d_h:
        raise ValueError(f"r numel {r.numel()} != u numel {d_h}")
    if five_d != 5 * d_h:
        raise ValueError(f"wx last dim {five_d} != 5 * d_h={5 * d_h}")
    ...
    validate_cuda_tensors(wx, u, r, h0, name="newton_titans_fused")
    ...
    for it in range(max_iters):
        _titans_cell_local_scan_kernel[grid_td](
            h,
            wx,
            u,
            r,
        ...
        if h32 is not None and r32 is not None:
            ...
            if incl_r is None:
                raise RuntimeError("fused Titans scan buffer missing for n_chunks>1")
            incl_r.copy_(scan_chunk_aggregates(agg_j, agg_r))
            _titans_apply_update_kernel[grid_td](
                h,
                j_loc,
                r_loc,
                incl_r,
