"""Fused ParaNLRU Newton: cell + diagonal J + scan (Alg. 1).

``W_x(x)`` is ``(B, T, 2 d_h)`` = gate + candidate affines. Recurrent mix
...
@triton.jit
def _nlru_pred_j(h_prev, ax, cx, u):
    """NLRU step and diagonal J: a + (1-a) * (1-n^2) * u."""
    a = tl.sigmoid(ax)
    n = _tanh(cx + u * h_prev)
...
def _load_wx(wx_ptr, pid_b, offs_t, offs_d, d_h, mask, sb, st, sd):
    ...
    ax = load_acc(base + offs_d[None, :] * sd, mask, 0.0)
    cx = load_acc(base + (offs_d[None, :] + d_h) * sd, mask, 0.0)
    return ax, cx


@triton.jit
def _nlru_init_kernel(
    wx_ptr,
    h_ptr,
    u_ptr,
    h0_ptr,
    ...
    dmask = offs_d < d_h
    ax, cx = _load_wx(wx_ptr, pid_b, offs_t, offs_d, d_h, mask, stride_wb, stride_wt, stride_wd)
    ...
    h_new, _ = _nlru_pred_j(h_prev, ax, cx, u)
    store_acc(
...
@triton.jit
def _nlru_cell_local_scan_kernel(
    h_ptr,
    wx_ptr,
    u_ptr,
    h0_ptr,
    ...
    ax, cx = _load_wx(wx_ptr, pid_b, offs_t, offs_d, d_h, mask, stride_wb, stride_wt, stride_wd)
    u = load_acc(u_ptr + offs_d, dmask, 0.0)
    h_new, j = _nlru_pred_j(h_prev, ax, cx, u)
    residual = h_new - h
    if HAS_CU:
...
def _newton_nlru_fused_impl(
    ...
    *,
    ...
    """Alg. 1 for diagonal ParaNLRU. Public entry: ``pararnn::newton_nlru_fused``."""
    wx = wx.contiguous()
    u = u.contiguous()
    batch, time, two_d = wx.shape
    d_h = u.numel()
    if two_d != 2 * d_h:
        raise ValueError(f"wx last dim {two_d} != 2 * d_h={2 * d_h}")
    ...
    validate_cuda_tensors(wx, u, h0, name="newton_nlru_fused")
    ...
    for it in range(max_iters):
        _nlru_cell_local_scan_kernel[grid_td](
            h,
            wx,
            u,
            h0,
        ...
        if h32 is not None and r32 is not None:
            ...
            if incl_r is None:
                raise RuntimeError("fused NLRU scan buffer missing for n_chunks>1")
            incl_r.copy_(scan_chunk_aggregates(agg_j, agg_r))
            _nlru_apply_update_kernel[grid_td](
                h,
                j_loc,
                r_loc,
                incl_r,
