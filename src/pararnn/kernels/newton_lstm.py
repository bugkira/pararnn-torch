...
def _load_wx(wx_ptr, pid_b, offs_t, offs_d, d_h, mask, sb, st, sd, FP16: tl.constexpr):
    ...
    fx = load_acc(base + offs_d[None, :] * sd, mask, 0.0, FP16)
    zx = load_acc(base + (offs_d[None, :] + d_h) * sd, mask, 0.0, FP16)
    ox = load_acc(base + (offs_d[None, :] + 2 * d_h) * sd, mask, 0.0, FP16)
    return fx, zx, ox
...
def _lstm_cell_local_scan_kernel(
    ...
    c_prev = _load_state(
        s_ptr, pid_b, offs_tm1, offs_d, SLOT_C, mask_prev, stride_sb, stride_st, stride_ss, stride_sd, FP16
    )
    h_prev = _load_state(
    ...
    _store_state(r_loc_ptr, r0s, pid_b, offs_t, offs_d, SLOT_C, mask, stride_rb, stride_rt, stride_rs, stride_rd, FP16)
    _store_state(r_loc_ptr, r1s, pid_b, offs_t, offs_d, SLOT_H, mask, stride_rb, stride_rt, stride_rs, stride_rd, FP16)
    ...
    store_acc(agg_j_ptr + pid_b * stride_ajb + pid_c * stride_ajc + 0 * stride_ajk + offs_d * stride_ajd, agg00, dmask, FP16)
    store_acc(agg_j_ptr + pid_b * stride_ajb + pid_c * stride_ajc + 1 * stride_ajk + offs_d * stride_ajd, agg01, dmask, FP16)
    store_acc(agg_j_ptr + pid_b * stride_ajb + pid_c * stride_ajc + 2 * stride_ajk + offs_d * stride_ajd, agg10, dmask, FP16)
    store_acc(agg_j_ptr + pid_b * stride_ajb + pid_c * stride_ajc + 3 * stride_ajk + offs_d * stride_ajd, agg11, dmask, FP16)
    store_acc(agg_r_ptr + pid_b * stride_arb + pid_c * stride_arc + SLOT_C * stride_ars + offs_d * stride_ard, aggr0, dmask, FP16)
    store_acc(agg_r_ptr + pid_b * stride_arb + pid_c * stride_arc + SLOT_H * stride_ars + offs_d * stride_ard, aggr1, dmask, FP16)
...
def _chunk_incl_kernel(
    ...
    j00 = load_acc(agg_j_ptr + pid_b * stride_ajb + offs_c[:, None] * stride_ajc + 0 * stride_ajk + offs_d[None, :] * stride_ajd, mask, 1.0, FP16)
    j01 = load_acc(agg_j_ptr + pid_b * stride_ajb + offs_c[:, None] * stride_ajc + 1 * stride_ajk + offs_d[None, :] * stride_ajd, mask, 0.0, FP16)
    j10 = load_acc(agg_j_ptr + pid_b * stride_ajb + offs_c[:, None] * stride_ajc + 2 * stride_ajk + offs_d[None, :] * stride_ajd, mask, 0.0, FP16)
    j11 = load_acc(agg_j_ptr + pid_b * stride_ajb + offs_c[:, None] * stride_ajc + 3 * stride_ajk + offs_d[None, :] * stride_ajd, mask, 1.0, FP16)
    r0 = load_acc(agg_r_ptr + pid_b * stride_arb + offs_c[:, None] * stride_arc + 0 * stride_ars + offs_d[None, :] * stride_ard, mask, 0.0, FP16)
    r1 = load_acc(agg_r_ptr + pid_b * stride_arb + offs_c[:, None] * stride_arc + 1 * stride_ars + offs_d[None, :] * stride_ard, mask, 0.0, FP16)
    _, _, _, _, u0, u1 = tl.associative_scan((j00, j01, j10, j11, r0, r1), 0, _compose_block2)
    store_acc(incl_r_ptr + pid_b * stride_ib + offs_c[:, None] * stride_ic + 0 * stride_is + offs_d[None, :] * stride_id, u0, mask, FP16)
    store_acc(incl_r_ptr + pid_b * stride_ib + offs_c[:, None] * stride_ic + 1 * stride_is + offs_d[None, :] * stride_id, u1, mask, FP16)
...
def _lstm_apply_update_kernel(
    ...
    mask = (offs_t[:, None] < time) & (offs_d[None, :] < d_h)
    dmask = offs_d < d_h
    j00 = load_acc(j_loc_ptr + pid_b * stride_jb + offs_t[:, None] * stride_jt + 0 * stride_jk + offs_d[None, :] * stride_jd, mask, 1.0, FP16)
    j01 = load_acc(j_loc_ptr + pid_b * stride_jb + offs_t[:, None] * stride_jt + 1 * stride_jk + offs_d[None, :] * stride_jd, mask, 0.0, FP16)
    j10 = load_acc(j_loc_ptr + pid_b * stride_jb + offs_t[:, None] * stride_jt + 2 * stride_jk + offs_d[None, :] * stride_jd, mask, 0.0, FP16)
    j11 = load_acc(j_loc_ptr + pid_b * stride_jb + offs_t[:, None] * stride_jt + 3 * stride_jk + offs_d[None, :] * stride_jd, mask, 1.0, FP16)
    r0 = _load_state(r_loc_ptr, pid_b, offs_t, offs_d, SLOT_C, mask, stride_rb, stride_rt, stride_rs, stride_rd, FP16)
    r1 = _load_state(r_loc_ptr, pid_b, offs_t, offs_d, SLOT_H, mask, stride_rb, stride_rt, stride_rs, stride_rd, FP16)
    ...
    c1 = load_acc(incl_r_ptr + pid_b * stride_ib + idx_c * stride_ic + SLOT_H * stride_is + offs_d * stride_id, dmask, 0.0, FP16)
    ...
    h = _load_state(s_ptr, pid_b, offs_t, offs_d, SLOT_H, mask, stride_sb, stride_st, stride_ss, stride_sd, FP16)
    _store_state(
        s_ptr, c + omega * delta_c, pid_b, offs_t, offs_d, SLOT_C, mask, stride_sb, stride_st, stride_ss, stride_sd, FP16
    )
    _store_state(
        s_ptr, h + omega * delta_h, pid_b, offs_t, offs_d, SLOT_H, mask, stride_sb, stride_st, stride_ss, stride_sd, FP16
    )
