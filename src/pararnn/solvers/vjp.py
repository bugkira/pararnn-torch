...
ParaGRU / ParaLSTM: closed-form elementwise VJP + one ``W_x`` GEMM (same split
as the fused forward). On CUDA the elementwise part is a Triton kernel.
Custom cells use Autograd on ``step`` — that is the generic path.
"""
