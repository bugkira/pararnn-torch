...
| Eisenstat & Walker, inexact Newton forcing terms | [SIAM J. Sci. Comput. 1996](https://doi.org/10.1137/0917003) | Adapt **linear** solve tolerance from residual ratios. **Not** our Picard P (we already exact-scan \(J\delta=-F\)). See [`next.md`](next.md). |
...
5. **Multi-GPU:** data-parallel SGD works for sequential FlashRNN and for Newton. Sequence-parallel *time* tiles need an associative scan carry (this repo: `scan_diag_two_ranks`). Informal “FlashRNN cannot use many cards” is DDP-false. See [`tex/seq-parallel-pararnn.tex`](tex/seq-parallel-pararnn.tex).
