...
Our `NewtonConfig(scan_backend="fused")` is a **Triton** reimplementation of paper Alg. 1 (cell + J + scan; `W_x` still a PyTorch GEMM). It is not a port of their `csrc/fused_*.cu`. Fused is handwritten for ParaGRU/ParaLSTM and ParaSLSTM `mix='diag'`, not any \(f\). Library default is `auto` (fused on CUDA for those cells, fp16/fp32, else Triton scan, else eager).
