...
`NewtonConfig(scan_backend="auto")`; fused kernels prepend `h0`; `NewtonStats` and residual early-stop (`NewtonDivergenceError` if max|F|>1 after K). Compile-safe preset: fixed-K Autograd path, fused Alg. 1 as `pararnn::newton_*_fused` custom ops. `ParaRNN` takes a cell or a list, `return_hidden`, LSTM `output_hidden`. **ParaSLSTM** `mix='diag'` is the fused 4×4 path; `mix='head'` is an unfused ablation (`scan_dense`); `mix='dense'` is a `d_h<=8` test oracle. **ParaGRU** `mix='diag'` is fused; `mix='head'` is Dreamer-style block-diagonal Cho-GRU (CUDA factorized Newton; `scan_backend='eager'` dense-J oracle; LN for Dreamer LN-GRU is outside the cell).
