...
Working today (vectorized PyTorch; optional Triton scan and **fused Newton**):

- Diagonal **ParaGRU** and CIFG peephole **ParaLSTM** (paper §3), plus **any** ``step(h, x)`` cell: Autograd Jacobian (DEER). Default is analytic J when the cell provides it.
...
- Numerics tests: analytic Jacobians vs autograd; parallel vs sequential; Newton grads vs sequential BPTT; custom diag/dense cells; packed VJP vs Autograd; Triton diag/2×2 scans and fused Newton vs substitution / sequential; **fp16 fused vs sequential separately** (atol \(2\times10^{-3}\); fused rejects bf16)
...
Python 3.10+, [uv](https://docs.astral.sh/uv/), CUDA PyTorch (cu128 index in `pyproject.toml`).
...
On a machine with several GPUs, `experiment_device()` selects by **name** (default `"2080 Ti"`), not `cuda:0`. Override with `PARARNN_DEVICE=cuda:1`. Turing cards have no bf16 tensor cores; tests run float32 and fp16 (not bf16).
...
import torch
from pararnn import (
    ParaGRU, NewtonConfig, experiment_device,
    newton_apply, sequential_apply, sequential_apply_compiled,
)

device = experiment_device()  # or torch.device("cpu") via experiment_device(allow_cpu=True)
cell = ParaGRU(d_in=32, d_h=64).to(device)
x = torch.randn(4, 128, 32, device=device)

h_seq = sequential_apply(cell, x)
h_par = newton_apply(cell, x, NewtonConfig(max_iters=3))
# h_par ≈ h_seq  (see tests/numerics/)
# sequential_apply_compiled(cell, x)  # honest timing baseline (#12)
# NewtonConfig(max_iters=3, scan_backend="fused")  # CUDA; cell+J+scan in Triton
# Any nn.Module with step(h, x) and d_h: NewtonConfig(jacobian="autograd")
# Mixing hidden channels: also jac_structure="dense"
```

`ParaLSTM` uses the same solvers. State shape is `(batch, time, 2, d_h)`.
...
- [ ] MLflow training runs; SlimPajama baselines at small scale
